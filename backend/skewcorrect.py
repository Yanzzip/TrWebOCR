...
class ImgCorrect():
    ...
    def search_lines(self):
        ...
        for x in lines:
            ...
            if 0 < degrees(atan((x[3] - x[1]) / (x[2] - x[0]))) < 45:
                if 0 < degrees(atan((x[3] - x[1]) / (x[2] - x[0]))) < 5:
            ...
            if -45 < degrees(atan((x[3] - x[1]) / (x[2] - x[0]))) < 0:
                if -5 < degrees(atan((x[3] - x[1]) / (x[2] - x[0]))) < 0:
    ...
    def rotate_image(self,degree):
        ...
        cv2.imwrite("imgRotation111.jpg", imgRotation)
        # cv2.imwrite("./image/r14a.jpg", imgRotation)
        return imgRotation
 
if __name__ == "__main__":
    im = cv2.imread("C:/Users/DELL/Desktop/z.jpg")
